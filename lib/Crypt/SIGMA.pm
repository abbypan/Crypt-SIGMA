...
sub b_recv_msg3 {
  ...
  my $b_mac4 = $mac_func->( $key_r->{km}, $b_tbm4 );
...
sub a_recv_msg2 {
  ...
  my (
    $group,    $msg2, $na, $ek_key_a_r, $hash_name, $key_len, $pack_msg_func, $unpack_msg_func, $mac_func, $sig_verify_func,
    $dec_func, $ctx
  ) = @_;
  ...
  my $key_r = calc_ke_km( $ek_key_a_r->{priv_pkey}, $a_recv_ek_b_pub_pkey, $hash_name, $key_len );
...
sub b_send_msg2 {
  ...
  my (
    $group,     $msg1,     $id_b, $random_range, $point_compress_t, $hash_name, $key_len, $pack_msg_func, $unpack_msg_func, $mac_func,
    $sign_func, $enc_func, $ctx
  ) = @_;
  ...
  my $ek_key_b_r = generate_ec_key( $group, $point_compress_t, $ctx );

  # $b_tbm = [1, na, B, g^y]
  my $b_tbm = $pack_msg_func->( [ 1, $b_recv_na, $id_b, $ek_key_b_r->{pub_bin} ] );

  my $key_r = calc_ke_km( $ek_key_b_r->{priv_pkey}, $b_recv_ek_a_pub_pkey, $hash_name, $key_len );
...
sub a_send_msg1 {
  ...
  my $ek_key_a_r = generate_ec_key( $group, $point_compress_t, $ctx );
...
sub generate_ec_key {
  my ( $group, $point_compress_t, $ctx ) = @_;
  $point_compress_t //= 2;

  my $ek = Crypt::OpenSSL::EC::EC_KEY::new();
  $ek->set_group( $group );
  $ek->generate_key();

  my $ek_priv      = $ek->get0_private_key();
  my $ek_priv_pkey = evp_pkey_from_priv_hex( $group, $ek_priv->to_hex );

  my $ek_pub      = $ek->get0_public_key();
  my $ek_pub_hex  = Crypt::OpenSSL::EC::EC_POINT::point2hex( $group, $ek_pub, $point_compress_t, $ctx );
  my $ek_pub_bin  = pack( "H*", $ek_pub_hex );
  my $ek_pub_pkey = evp_pkey_from_point_hex( $group, $ek_pub_hex, $ctx );

  return {
    priv_pkey => $ek_priv_pkey, priv_key => $ek, priv_bn => $ek_priv, pub_point => $ek_pub, pub_hex => $ek_pub_hex,
    pub_bin   => $ek_pub_bin,   pub_pkey => $ek_pub_pkey
  };
